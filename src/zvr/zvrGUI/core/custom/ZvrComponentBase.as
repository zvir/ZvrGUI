package zvr.zvrGUI.core.custom
{
	...
	public class ZvrComponentBase extends EventDispatcher implements IZvrComponent
	{
		...
		public function ZvrComponentBase(skinClass:Class, bodyClass:Class)
		{
			...
			_behaviors = new ZvrComponentBehaviors(this);
			_states = new ZvrStatesManager(this);
			_presents = new ZvrStatePresentsManager(this, setVisible);
			
			defineStates();
			
			_skin = new skinClass(this, skinRegistration); // todo think of dinamicly replecing skins
			_skinCreate();
			
			if (_skin.body && _skin.body != _body)
				_body.addElement(_skin.body);
			if (_skin.shell && _skin.shell != _body)
				_body.addElement(_skin.shell);
		...
		{
			dispatchEvent(new ZvrComponentEvent(type, this, delta));
		}
		...
		{
			return _body.addElement(child);
		}

		public function removeChild(child:Object):Object
		{
			return _body.removeElement(child);
		}
		...
		public function setStyle(styleName:String, value:*, state:* = null):void
		{
			if (_skin.setStyle(styleName, value, state))
				dispatchEvent(new ZvrStyleChangeEvent(ZvrStyleChangeEvent.CHANGE, styleName, value));
		}
