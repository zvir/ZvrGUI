...
package zvr.maxRect
{
	...
	public class MaxRectsBinPack extends EventDispatcher
	{
		...
		public var binWidth:Number = 0;
		public var binHeight:Number = 0;
		
		protected var enterFrameProvider:Shape;
		protected var tmpRects:Vector.<Rectangle>;
		protected var resRects:Vector.<Rectangle>;
		...
		public function insertBulk(rects:Vector.<Rectangle>, results:Vector.<Rectangle>, method:uint):void
		{
		...
		public function insertBulkWorker(event:Event):void
		{
			...
			while (tmpRects.length > 0)
			{
				if (getTimer() - t < timeLimit)
				{
					...
					if (bestRectIndex == -1)
					{
						trace(" Rect didn't fit, break out of the loop and terminate");
						break;
					}
					
					resRects[bestRectIndex].x = bestNode.x;
					resRects[bestRectIndex].y = bestNode.y;
					resRects[bestRectIndex].width = bestNode.width;
					resRects[bestRectIndex].height = bestNode.height;

					placeRect(bestNode);
					tmpRects.splice(bestRectIndex, 1);
					resRects.splice(bestRectIndex, 1);
				}
				else
				{
