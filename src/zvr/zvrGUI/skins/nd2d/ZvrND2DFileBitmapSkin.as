package zvr.zvrGUI.skins.nd2d 
{
	...
	import zvr.zvrND2D.DynamicFileTexture;
	import zvr.zvrND2D.DynamicFileTextureEvent;
	 ...
	 * @author Zvir
	 */
	public class ZvrND2DFileBitmapSkin extends ZvrSkin 
	{
		private var _dynamicTexture:DynamicFileTexture;
		
		public function ZvrND2DFileBitmapSkin(component:IZvrComponent, registration:Function) 
		{
		...
		override protected function create():void 
		{
			...
			_dynamicTexture = new DynamicFileTexture(null, _body as ZvrND2DSkin);
			
			_dynamicTexture.addEventListener(DynamicFileTextureEvent.READY, ready);
			_dynamicTexture.addEventListener(DynamicFileTextureEvent.PROGRESS, progress);
			_dynamicTexture.addEventListener(DynamicFileTextureEvent.LOADING, loading);
		...
		{
			registerStyle(ZvrND2DStyles.FILE_PATH, updateTexture);
		...
		private function updateTexture():void 
		{
			...
			t = getStyle(ZvrND2DStyles.FILE_PATH);
			
			if (t)
			{
				_dynamicTexture.path = t;
			}
		...
		private function progress(e:DynamicFileTextureEvent):void 
		{
			_component.dispatchEvent(e);
		}
		
		private function ready(e:DynamicFileTextureEvent):void 
		{
			updateTextureSize();
			_component.dispatchEvent(e);
		}
		
		private function loading(e:DynamicFileTextureEvent):void 
		{
			_component.dispatchEvent(e);
		}
