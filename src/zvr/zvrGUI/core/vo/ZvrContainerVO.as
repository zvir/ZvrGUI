package zvr.zvrGUI.core.vo
{
	...
	import zvr.zvrGUI.core.flash.ZvrFlashContainerRelay;
	...
	public class ZvrContainerVO extends ZvrComponentVO
	{
		...
		private var _contents:IZvrContainerContents;
		protected var _mask:IZvrContainerMask;	
		...
		private var addChild:Function;
		private var removeChild:Function;

		private var _bodyContainer:IZvrContainer;
		
		public function ZvrContainerVO(skinClass:Class, body:IZvrContainer, mask:IZvrContainerMask, contents:IZvrContainerContents, addComponentObject:Function, removeComponentObject:Function)
		{
			_contents = contents;
			_mask = mask;
			_contents.addMask(_mask);
			_bodyContainer = body;
			addChild = addComponentObject;
			removeChild = removeComponentObject;

			super(skinClass, body);

			_contentPadding = new ZvrContentPadding(contentPaddingSetter);

			_body.addEventListener(ZvrComponentEvent.RESIZE, resized);
		...
		*/
		...
		private function updatePresentElement(component:IZvrComponent):void
		{
		...
		private function setContentSize(w:Number, h:Number):void
		{
			...
			_body.dispatchEvent(new ZvrContainerEvent(ZvrContainerEvent.CONTENT_SIZE_CHANGE, _bodyContainer, null));
		}
		
		protected function updateMask():void
		{
			_mask && _mask.redrawMask(_contentPadding.left, _contentPadding.top, bounds.width - _contentPadding.right - _contentPadding.left, bounds.height - _contentPadding.bottom - _contentPadding.top);
		}
		...
		private function contentPaddingSetter(top:Number, left:Number, right:Number, bottom:Number):void 
		{
			if (top != 0 || left != 0) 
			{
				_contents.x -= left;
				_contents.y -= top;
				_body.dispatchEvent(new ZvrContainerEvent(ZvrContainerEvent.CONTENT_POSITION_CHANGE, _bodyContainer, null));
				updateContainer();
			} 
		...
		public function addComponent(element:IZvrComponent):IZvrComponent
		{
			...
			element.addToContainer(_bodyContainer);
			updatePresentElement(element);
			
			updateContainer();

			_body.dispatchEvent(new ZvrContainerEvent(ZvrContainerEvent.ELEMENT_ADDED, _bodyContainer, element));
		...
		public function removeComponent(element:IZvrComponent):IZvrComponent
		{
			...
			element.removeFromContainer(_bodyContainer);

			updateContainer();

			_body.dispatchEvent(new ZvrContainerEvent(ZvrContainerEvent.ELEMENT_REMOVED, _bodyContainer, element));
		...
		public function setElementIndex(child:IZvrComponent, index:int):void
		{
			...
			var i:int = getElementIndex(child as IZvrComponent);
		...
		public function setLayout(layout:Class):void
		{
			...
			_layout = new layout(_body, computeContentSize, registerLayout, setContentAreaIndependent);
			updateLayout();
		}
		...
		public function get contentssssssss():IZvrContainerContents
		{
			return _contents;
		}
